...
[![Version](https://img.shields.io/badge/Version-0.3.1-8A2BE2)](src/memoria/__version__.py)
...
- **多目标与语义边**：一个链接文本可指向多个目标——点击按队首跳转、其余进标签栏备选；每条边带类型（引用 / 扩展 / 包含）与关联强度，随图谱构建同步，可逐条抑制或删除。
...
- **样式笔刷与取色器**：点选样式（加粗 / 斜体 / 高亮 / 字体色）后左键拖动即可连续涂抹应用，右键取消单项；取色面板支持自定义色板（可增删颜色）。
...
- **候选与建议**：正文提及、别名与描述以「候选」形式呈现，可一键采纳为别名 / tag / 描述；「系统建议 / 同步检索 aux」从正文索引生成建议候选。
...
- **界面语言（中 / EN）**：设置 → 显示 → 界面语言，即时切换并自动记忆；全部界面文案双语化，英文缺失项自动回退中文。
...
- 版本号唯一事实源：[src/memoria/\_\_version\_\_.py](src/memoria/__version__.py)（当前 **0.3.1**）
