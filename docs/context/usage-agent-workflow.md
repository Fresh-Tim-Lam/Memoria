...
> **关联文档**：[standards/import-format.md](../standards/import-format.md)（平面文件格式规范）；[prompts/role-a-content-agent.md](../prompts/role-a-content-agent.md)（角色 A 完整手册）；[standards/markdown-form-std.md](../standards/markdown-form-std.md)（`[[]]` 语法体系）；[standards/docs-management.md](../standards/docs-management.md)（docs 自维护规则）。
...
把用户提供的原始知识文本整理为 **Memoria 平面导入格式**，严格遵守 [standards/import-format.md](../standards/import-format.md)。
...
字段规则与 import-format.md 一致；正文标题用 `##`/`###`，链接用 `[[id]]`、`[[id|文本]]`、`[[id#type]]`（语法见 [markdown-form-std.md](../standards/markdown-form-std.md)）。
...
1. **先改规范**：平面格式 → `standards/import-format.md`；`[[]]` 语法 → `standards/markdown-form-std.md`
2. **再同步角色 A 手册**：`prompts/role-a-content-agent.md`（其第 9 节已声明以规范为准，但内容应同步避免误导）
3. **再同步本文档**：本流水线规则、质量标准随之修订
4. **登记**：在 `standards/docs-management.md` §4.2 规范修订记录登记一行（日期/文档/修订内容）
