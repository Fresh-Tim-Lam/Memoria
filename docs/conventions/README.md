...
| [ledger-maintenance.md](./ledger-maintenance.md) | Ledger maintenance rules: single source of truth, status markers + four categories (open / code-done-acceptance-pending / review-pending / closed), mandatory evidence anchors, closure criteria, periodic reconciliation | Anyone registering or updating todo / gate / maintenance-job ledger entries (`registrar`) |
| [frontend-modules.md](./frontend-modules.md) | Frontend JS modularity: new features must not go into `app.js`, explicit dependency injection via `init({...})`, shared pure helpers live in a core file (no private cross-file exports), "extract on touch" incremental refactor, self-check list | Anyone adding or modifying frontend code (`src/memoria/ui/static/**`) |
