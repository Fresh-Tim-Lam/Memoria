...
| 2026-09-16 | example/AAA_Vocab 公开 + Pages | 公开 IELTS 词汇库（`docs/example/AAA_Vocab/`）：`.gitignore` 政策由「仅展示 showcase 官方库」改为「showcase + AAA_Vocab」（运行时产物 `.memoria/agent|images`、`cache`、`pending.*`、`kp_targets`、`*.bak|log` 仍忽略）；新增 `.github/workflows/pages.yml`（GitHub Actions **只上传该目录**发布 Pages，站点根即指引页）；同步新增该库的上手/部署指引双版（`index.html` + `GUIDE.md`，12 张截图在 `images/`）与推荐提示词副本 `prompts/IELTS-vocab.md` |
