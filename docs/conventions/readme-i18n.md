...
| 2026-09-16 | §3.1 收录：**分辨率基线更新为 1536×816**（应用窗口客户区，宽高比 1.88；此前 1680×1000）。同日 8 张演示截图整批重拍：图谱两张取自「软件工程概论」知识库（先拖宽图谱面板、再把图谱移到区域中央），样式类取自仓库内示例库 `docs/example/showcase`；并同步修正两处与图不符的配图说明（预览不再声称荧光笔、公式不再声称 cases/align）。采集方式：宿主应用置前后由外部脚本抓窗口客户区（F9 触发，带重复帧识别）|
