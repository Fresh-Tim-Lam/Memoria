...
- 截图必须是**真实界面捕获**（自动化/手动均可），统一命名 `demo-<场景>.png`（当前：workspace / graph-3d / split / preview / math / settings / check / check-report），分辨率尽量一致（**2026-09-16 起：应用窗口客户区 1536×816，宽高比 1.88**；此前为 1680×1000，宽高比 1.68。自动捕获受宿主窗口/视口限制时，以**接近的宽高比**为准并在登记时注明实际像素）。
- 仓库自带采集工具 [scripts/screenshots/hotkey_capture.py](../../scripts/screenshots/hotkey_capture.py)：热键（默认 F9）触发抓窗口客户区、按序命名、重复帧不占号 —— 重拍时优先用它（用法见该目录 README），可避免"拍错区域 / 拍到上一页"这类系统性返工。
- 新增截图流程：
...
| 2026-09-16 | §3.1 收录：**分辨率基线更新为 1536×816**（应用窗口客户区，宽高比 1.88；此前 1680×1000）。同日 8 张演示截图整批重拍：图谱两张取自「软件工程概论」知识库（先拖宽图谱面板、再把图谱移到区域中央），样式类取自仓库内示例库 `docs/example/showcase`；并同步修正两处与图不符的配图说明（预览不再声称荧光笔、公式不再声称 cases/align）。采集方式：宿主应用置前后由 [scripts/screenshots/hotkey_capture.py](../../scripts/screenshots/hotkey_capture.py) 抓窗口客户区（F9 触发，带重复帧识别）|
