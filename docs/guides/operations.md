...
- **随包资源**：`Package/resources/` 由 `packaging/build.py` `_stage_runtime_resources()` 登记表收集（icons/agent-prompts/docs/白名单/examples/example-boonie），构建结束前自动自检必带文件（含 `resources/agent-prompts/organize.zh-CN.md`、`resources/docs/preview-formats.md`）。新增随包资源按 [packaging/README.md](../../packaging/README.md)「随包资源登记」四步维护；构建后自检：
  ```powershell
  python -c "from pathlib import Path; p=Path('Package/resources'); print((p/'agent-prompts'/'organize.zh-CN.md').is_file(), (p/'docs'/'preview-formats.md').is_file(), (p/'icons'/'Memoria.ico').is_file())"
  ```
