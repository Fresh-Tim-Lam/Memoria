...
- ⚠️ **本轮（2026-09-19：助手气泡渲染 Markdown）已断言 / 未取证**。已断言（**仓库自带 harness `docs/example/rich-content-test/_harness.py`，端口 8646**；`MEMORIA_HARNESS_KB` 指向临时库、`MEMORIA_CONFIG_DIR` 隔离；**不需要模型** —— 助手回复直接写进会话文件，再从 `#agent-history` 下拉载入）—— **20 项断言全通过、页面无 JS 异常**：
