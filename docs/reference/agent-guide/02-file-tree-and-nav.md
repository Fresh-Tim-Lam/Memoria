...
| 目录行 | file-tree.js:164-177 | 结构见上图；`twisty` = dsh `IconTriangleRightFill14` 内联 SVG，**展开时由 CSS 旋转 90°**（file-tree.js:169；样式在 app.css 末尾块）；目录图标 = `IconFolderOpen16`/`IconFolderClose16` 随展开态切换（file-tree.js:170）；缩进 `padding-left = 4 + depth×14` px（file-tree.js:166）。**2026-09-19**：缩进区新增逐层连接线（dsh `IconTreeCorner8x10`，`treeGuides(depth)`，file-tree.js:168、700-706；样式 `.-tree-guide-*` 在 app.css 末尾块）。**2026-09-22**：折角字形 `IconTreeCorner8x10` 只有 **8px** 宽而槽是 **14px** ⇒ 槽内右侧那 6px 由 `.-tree-guide-corner::after` 的 **1px 横线**延长到行首字形（`left: 7px; right: 0; top: calc(50% + 4.5px)`，颜色/透明度同字形 `currentColor` + `.75`）⇒ 折角**紧贴**行首字形（人：「折角符号应该紧贴文件图标」） |
| 文件行 | file-tree.js:179-189 | 缩进 `padding-left = 4 + depth×14` px（**2026-09-22 由 `12 + depth×14` 改回**：折角槽就是**1 整格 14px** ⇒ 行首字形（= 文件类型图标）必须落在**槽的右缘**才紧贴折角；旧值让文件图标比折角多空 **8px** —— 人：「你之前设计的那个『折角』符号应该紧贴『文件图标』，而不是空一点位置」）；`title` 为完整相对路径；`data-path` 为归一化路径 |
| 图标与侧车 | file-tree.js:180-186；app.css:3421-3428 + **末尾追加块 5522-5583** | **2026-09-19 起不再用 emoji**：文件图标按扩展名/文件名分类（`classifyFileType`，file-tree.js:689-694）取 dsh `FileTypeIcon` 的类目 glyph（markdown/code/image/html/pdf/ppt/video/word/excel/other），类目→图标名映射见 `FT_CATEGORY_ICON`（file-tree.js:683-686）。原「有侧车 `📄` / 无侧车 `📝`」的**双字形差异已取消**——该信息本就由 `.no-sidecar` 类（图标透明度 **0.55**；**2026-09-20 由 0.45 复校** —— dsh 单色 glyph 的 mark 层是细笔画，0.45 叠加后浅色主题下合成对比仅约 2.5:1、偏"几乎看不见"，改 0.55 约 3.3:1，清晰态仍是 0.85 ⇒ 两态一眼可辨；覆盖块 app.css:5601-5613）。**2026-09-22 起再补「颜色轴」**（人：「缺少元数据配置的 md 文档和已经配置的 md 文档的图标区分开（以前是另一个文档图标：配置过的有颜色，没配置的没颜色、浅一点）」）：**已配置 ⇒ 图标着 `--theme-color`；未配置 ⇒ `--text-secondary` 灰 + 保持 0.55**（覆盖块 app.css:6091-6112；两条**互斥**选择器 `.-tree-item:not(.no-sidecar) .-tree-icon` / `.-tree-item.no-sidecar .-tree-icon`，只作用于文件行 ⇒ 目录行的文件夹图标不受影响）。真机实测（harness 8643 + 临时库，52 行全覆盖）：`vocab/*` = `rgb(0,122,204)`@0.85（主题蓝）vs `docs/*` = `rgb(133,133,133)`@0.55（灰）⇒ 「有色 vs 灰且浅」一眼可辨；契约钉子 = `tests/test_file_tree_icons.py`。图标颜色一律 `currentColor`（随所在行的 `--text-secondary`/`--text-bright`，见末尾样式块） |
...
| 树样式 | app.css:3382-3429；**图标/连接线块末尾追加 5522-5583**；**两态配色块末尾追加 6091-6112**（2026-09-22）；**折角延长线块末尾追加 6197-6217**（2026-09-22） |
