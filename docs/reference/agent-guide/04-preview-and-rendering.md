...
- ⚠️ 待确认（结构缺口）：Mermaid 渲染成功/失败时生成的替换容器只复制了 `.-src-block` 与 `data--block-index`，**未复制 `data--src-line` / `data--src-line-end`**（`markdown-preview.js:205`-`213`、`215`-`222` vs 标记逻辑 `app.js:1816`-`1820`）。因此 mermaid 块对 `#-preview-range-band`、`_getViewTopSrcLine`（均按 `[data--src-line]` 筛选，`app.js:1647`、`1672`、`11150`）不可见。未做运行时验证。
