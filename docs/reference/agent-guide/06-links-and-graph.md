...
应用内对话的 agent 现在也能**只读地解析引用**（不是新的链接存储，也不改本文 §2.1–§2.9 的任何链路）：两把工具 `resolve_reference`（解一条）/ `audit_references`（查一批），实现 `services/agent/tools/kb.py:1608-2821`（追加块，§6.21 重取：原 `:1608-2710`），口径见 [../../design/dsh-agent-port.md §6.18](../../design/dsh-agent-port.md)（**区间读时投影见 §6.20；列号见 §6.21**）。
...
| `文件:行号`（含 `#L12-L30` / `#L3C2-L5C7`） | 答案里的 `-agent-anchor`（`agent-panel.js:117` 的 `ANCHOR_RE` 同源字符类）；源码/预览选区的「加入对话」写 `@路径#L12-L30`（源码区还带**列号** `#L3C2-L5C7`；`agent-panel.js:3456` `rangeTokenText()` / `:3509` `formatRangeMention()`） | 行号对照剥 frontmatter 的正文行（`tools/kb.py:175`）；**区间回 `ok`（含解析到的行范围 + 首末行摘要）、倒置回 `invalid`、任一端越界回 `not_found`**（L 路线读时投影，2026-09-20 起，`_anchor_range_result()`，`tools/kb.py:2696`；**列 1 起、缺列 = 行首/行末、列越界 ⇒ `not_found`、同行列倒置 ⇒ `invalid`**） |
