...
└── .toolbar-search-wrap                    index.html:71   app.css:1995-2009（宽 min(380px,42vw)）
    ...
    └── .-toolbar-search-field              index.html:76
        ...
        └── #toolbar-search-panel           index.html:78   结果面板，z-index 9000（app.css:2096）

图片（运行时创建，无静态 HTML 节点）
├── #-image-manager .-modal                 image-tools.js:240-259（z-index 1000，app.css:4125）
│   ├── #imgr-stat                          image-tools.js:251
│   ├── [data-act=refresh|diagnose|cleanup] image-tools.js:252-254
│   └── #imgr-grid .-image-mgr-grid         image-tools.js:256（app.css:3879-3887）
├── #-img-diagnose .-modal                  image-tools.js:286-318（诊断子弹窗）
└── .-lightbox-overlay                      markdown-preview.js:507（z-index 99999，app.css:4032-4033）
```
...
|---|---|---|---|---|
| 搜索范围开关 | `.toolbar-search-wrap` 左端 | `.-toolbar-search-scope`（app.css:2018-2027） | 整块可点 / 可聚焦；click 或 Enter/Space 翻转「全库 ⇄ 文件」（toolbar-search.js:253-263）；子按钮 `pointer-events:none`（app.css:2036） | 内部「文件」按钮无 `currentPath` 时 `disabled`（toolbar-search.js:60）、title 改为 `app.openFileFirst`；`aria-checked` 同步（toolbar-search.js:67） |
| 全库 / 文件 子按钮 | 同上 | `.-toolbar-search-scope-btn[.active]`（app.css:2029-2047） | 不单独响应点击（父级统一处理） | 当前态加 `.active`（蓝底白字） |
| 搜索框 | 高 1.5rem，虚线边框，聚焦转实线（app.css:2057-2080） | `.-toolbar-search` | 输入防抖 **220ms**（toolbar-search.js:245-252）；Enter = 搜索、Shift+Enter = 仅当前文件、Esc = 关面板并失焦、聚焦时有内容立即重搜（toolbar-search.js:265-277）；Ctrl+K 聚焦并全选（toolbar-search.js:33-38、281-286） | 未开库时状态栏报 `app.openKbFirst`（toolbar-search.js:132-135） |
...
| 项 | 证据 | 说明 |
|---|---|---|
| 定位 / 层级 | app.css:2082-2097 | 绝对定位于搜索框正下方（`top:calc(100%+2px)`），`max-height:min(240px,40vh)`、`overflow-y:auto`，**`z-index:9000`** |
...
| 元素 | 证据 | 说明 |
|---|---|---|
| 弹窗骨架 | image-tools.js:240-259 | 运行时创建 `#-image-manager`（类 `.-modal`），box 类 `.-image-mgr-box`（宽 `min(760px,94vw)`、高 ≤82vh，app.css:3853-3858） |
...
| 列表 | image-tools.js:337-384 | **单一扁平网格**（`grid-template-columns: repeat(auto-fill, minmax(210px,1fr))`，app.css:3883-3884），无「引用中 / 未使用 / 可清理」三个分区；分区语义只由卡片名称行内紧跟的 `.-img-tag` 徽标表达：`.-img-tag-used`「已引用 {n}」/ `.-img-tag-unused`「未使用」（image-tools.js:354-356；样式 app.css:3950-3960）；卡片含缩略图、名、格式化体积、引用文案（`img.refsUsed` / `img.refsUnused`）与「插入」「删除」 |
...
| 控件 | 证据 | 行为 |
|---|---|---|
| 对齐 left/center/right | edit-handler.js:963-966、1193-1206 | 三按钮 `.-img-align-btn[data-align]`；当前值加 `.active`（app.css:3968-3971）；点击 → `memoria:image-attr{align}` |
...
| 默认尺寸钳制 | app.css:3841-3845 | 未显式给尺寸时 `.-preview img { max-width:35% }` |
| 图片块编辑外观 | app.css:3963-3967 | 编辑中图片加蓝色虚线 outline + `cursor:pointer` |
| caption 样式 | app.css:3988-4015 | 默认灰色 0.875rem；编辑中加 `.-caption-editing`（虚线框 + 淡蓝底）；input 透明无边框、继承字体 |
| Lightbox | markdown-preview.js:496-519；app.css:4031-4048 | 对容器内每个 `img` 绑 `dblclick`：**运行时新建** `.` `-lightbox-overlay`（fixed、`inset:0`、`z-index:99999`、黑 85%）+ `img.-lightbox-image`（`max-width/height:95vw/vh`）；点击遮罩即 `remove()`；图片光标改为 `zoom-in` |
...
| 修复过的 WebView2 缺陷 | image-tools.js:371-374；app.css:3898-3900 | ① 缩略图不用 innerHTML 解析而是 `new Image()` 再插；② 卡片**不得**加 `overflow:hidden`（flex + overflow:hidden 会导致已加载图片不绘制） |
...
1. **搜索面板层级高于所有弹窗**：面板 `z-index:9000`（app.css:2096）> `.-modal` 的 `1000`（app.css:4125）。弹窗不锁焦点，弹窗打开时 Ctrl+K 仍可聚焦顶栏搜索框（快捷键无弹窗判断，toolbar-search.js:281-286），此时结果面板会盖在弹窗之上。
...
| 搜索框与面板 CSS（含 z-index 9000） | app.css:1995-2110 |
| 结果条目 CSS（分数 / 分节标签 / 两行主列） | app.css:1264-1351 |
...
| 图片相关 CSS（尺寸 / 对齐 / 管理 / 编辑 / caption / Lightbox） | app.css:3840-4048 |
...
- ⚠️ 待确认（未能取证）：`img.cleanup.doneList`（zh-CN.js:1076）在前端未见调用点，疑似遗留键；`img.menuLabel`（zh-CN.js:1007）同样未在 `js/**` 中检出。
