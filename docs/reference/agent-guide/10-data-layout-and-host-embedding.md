...
└── .memoria/                              隐藏目录：被扫描器整体跳过（storage/constants.py:11-17）
    ...
    └── agent/                             Trae 智能体工具包（services/kb_agent.py:40-42）
        ...
        └── sessions/                     **应用内对话的会话事实源**（JSONL，append-only）
            └── <session-id>.jsonl        session/store.py:56-66、169-237（M1b 起由 `ask()` 写；M1c 起可被面板历史下拉**只读**列出/载入；**M1 收尾起可被 `agent_session_delete` 删除**；**M2 起事件面新增 `compaction`（长会话压缩）记录**——产品**唯一**允许写知识库的位置，见 §2.15）
```
...
### 2.15 应用内对话：9 个 RPC、会话事实源、多轮续聊、长会话压缩、会话检索与「停止/删除」（2026-09-17 落地，2026-09-18 增补 M1c、M1 收尾与用量统计，2026-09-19 增补 M2 压缩与会话检索工具）
...
**M2（2026-09-19）起**：会话事件面新增 `compaction` 记录（**纯追加**，**不新增 RPC、不 bump `SESSION_FORMAT_VERSION`**），见下方「长会话压缩」；同轮新增**第 6 个只读知识库工具** `search_sessions`（**也不新增 RPC** —— 它只在模型回合内部可用，前端不调用），见下方「会话检索工具」。
...
| `agent_ask_poll` | `(job_id: str, cursor: int = 0)` | `{status:"running"\|"done"\|"error", job_id, delta, cursor, answer, anchors, tool_calls, usage, session_id, stop_reason, iterations, error, code, cancelled, elapsed_ms}`；未知作业返回 `code:"unknown_job"`。**`cancelled` 为 M1 收尾追加字段**（只增）：被停止的作业为 `status:"done"` + `stop_reason:"aborted"` + `cancelled:true` + `answer`=已生成片段 |
...
| `agent_sessions_list` | `(kb_path: str \| None = None)` | `{status:"ok", sessions:[{session_id, modified_at(epoch 毫秒), turn_count, preview(前 80 字), title(前 40 字), capped}], count}`；按文件修改时间倒序、**最多 30 条**（`SESSION_LIST_LIMIT`，ui.py:1251）；`capped` 为 **M1 收尾追加字段**（见下「去读放大」）。单份会话损坏只跳过该条。**只读、不写盘** |
...
- **偏好落点（M1 收尾；2026-09-18 改为按库记）**：`config/ui-settings.json` 的**顶层 `agent` 段**里的 `lastSessionByKb`——`{ "<库标识>": "<会话 id>" }` 的映射，**每个知识库各自记一份**自己的「上次会话」（提问成功 / 手动载入 / 自动恢复时写入；**清空对话时只删本库条目**，不动别的库）。库标识由 `kbKey()`（agent-panel.js:326-329）把知识库路径归一化后得到：`/`→`\`、去尾分隔符、Windows 上再 `toLowerCase()`（NTFS 大小写不敏感），读/写/迁移/清理**全部**经它，避免同一库分裂成多条。读写统一走 `readAgentPrefs()`（:332-340）、`setLastSessionId()`（:372-385）、`clearLastSessionId()`（:388-403）、`writeAgentPrefs()`（:361-366，两步写见 §2.9），**绝不覆盖 `layout` 段**。**旧版全局单键 `agent.lastSessionId` 兼容策略＝一次性迁移**：`restoreLastSession()`（:1028-1045）在映射里没有本库条目但存在旧键时，把旧键当候选，**只有它在当前库下确实能载入时才认领**（写入映射并删除旧键），载入失败则不迁移、不删除（留给它原本所属的库认领）；此后不再读写旧键。
...
- **会话事实源**：`<kb>/.memoria/agent/sessions/<session-id>.jsonl`（`services/agent/session/store.py:56-66`）。首行 header、其后每行一个事件（`seq` 连续、append-only、撕裂尾部对读者不可见）；由 `ask()` 写（ask.py:282-316）。事件类型既有 7 类（`user/message` / `assistant/message` / `tool/call` / `tool/result` / `step/start` / `step/error` / `loop/end`，写入点 loop.py:302-398 与 ask.py:297），**M2（2026-09-19）起新增第 8 类 `compaction`**（形状与回放口径见下「长会话压缩」）。`session_id` 随 `agent_ask_poll` 的最终结果返回，面板状态行显示。
- **多轮续聊（M1c）**：`session_id` 指向的会话文件**已存在**时，`ask()` 先按 `session/history.py::build_history()`（history.py:307-322）把它回放成消息序列，作为 `messages=` 传给 `loop.run(question, messages=history)`（ask.py:273-280、315）——**回放发生在追加本轮 `user/message` 之前**，故历史里不含本轮问题、不会被重复加入；`session_id` 省略或文件不存在 ⇒ 全新会话，行为与 M1b 一致；`ask(replay=False)` 可显式关闭回放（测试/脚本用，ask.py:246）。事件 → 消息的映射：`user/message`→user、`assistant/message`→assistant（含 `tool_calls`）、`tool/result`→tool（`tool_call_id` 对齐），`tool/call`/`step/*`/`loop/end` 跳过；`compaction` 按下方「长会话压缩」的口径回放（摘要替换被覆盖区间）。
- **回放保真度（全保真，仅异常轮降级）**：`ToolCall` 可用 `{id,name,arguments}` 完整重建，且 OpenAI 兼容适配器正是按 `tool_call_id`/`tool_calls` 序列化（`llm/providers/openai_compatible.py:374-391`），故**带工具的轮次原样回放**（assistant 带 tool_calls + 紧随其后的 tool 消息）。唯一降级分支：某条 assistant 的 `tool_calls` 与随后的 `tool/result` **无法一一配对**（`id` 为空/数量不等/顺序不符，正常写作只在进程崩在工具执行中途出现）⇒ **整轮丢弃**，保证输出里绝不出现孤立 `tool_calls` 或孤儿 `tool` 消息（违规会被端点 400）。**容量上限与截断策略**：最多 **40 条消息**（`MAX_HISTORY_MESSAGES`，history.py:89）/ **32000 字符**（`MAX_HISTORY_CHARS`，history.py:91），**从最新往旧保留**整条消息，至少保留最新 1 条（单条超预算也保留，否则续聊会凭空丢掉当前上下文）；截断后若首部是被截掉 assistant 的 `tool` 消息，则一并丢弃以保持序列合法（`_truncate()`，history.py:267-281）。**M2 起截断退居兜底**：`build_history()` 先应用 `compaction` 压缩、再做上述截断（history.py:307-322）。
- **长会话压缩（M2，2026-09-19；`compaction` 事件）**：新模块 `services/agent/compaction.py`（**纯标准库**，语义移植自上游 `packages/compaction/compaction` + `compaction-basic`，pin `0d1f5000`）。阈值/保留是**字符预算**（本地无「上下文窗口」概念，故用比例乘字符预算；上游是比例乘 context window）：触发阈值 `compact_threshold_chars()` = `MAX_HISTORY_CHARS(32000) × COMPACT_THRESHOLD_RATIO(0.8)` = **25600 字符**，尾部**逐字保留** `retain_chars()` = `× RETAIN_RATIO(0.16)` = **5120 字符**（compaction.py:104-106、187-197）。其余常量：`MIN_SPAN_CHARS = 2000`（**本地新增**下限）、`SUMMARY_MAX_TOKENS = 8192`、`SUMMARY_RETRY_POLICY`（`max_retries=1`、`total_timeout_s=60`）、`SUMMARY_OPEN_TAG`/`SUMMARY_CLOSE_TAG`、`CHECKPOINT_PREAMBLE`、`COMPACTION_INSTRUCTION`（8 段结构化 Markdown 骨架的中文落法，compaction.py:103-171）。
  - **区域选择（`select_span()`，compaction.py:259-297）**：被压缩区间恒为**前缀**（最旧那一段）；切割点必须**工具配对平衡**（`balanced_cuts()`，compaction.py:240-256 = 上游 `toolPairingBalancedBefore` 的「未闭合工具调用数」折叠，不得落在 assistant 的 `tool_calls` 与其 `tool/result` 之间）；满足「尾部 ≥ 5120 字符」时**尽量多压**（取最靠后的合法切割点）；另有两条本地下限：覆盖量 ≥ 2000 字符、区间内至少含一条 `user/message`。
  - **摘要调用（`summarize_span()`，compaction.py:300-366）**：**同一份 `system` + 同一份 `tools` + 被覆盖区间的消息**，把 `COMPACTION_INSTRUCTION` 作为**最后一条 `user` 消息**追加（上游理由：让这次辅助调用成为上一次已路由请求的真实前缀，从而复用 provider 的 KV 缓存，`summarizer.ts:24-30`）；摘要固定 **8 段结构化 Markdown**、空段写 `(none)`、遇到旧 `<compacted-summary>` 要**合并**而非照抄。
  - **fail-closed**：无终止事件 / `error` / `aborted` / `max-tokens` / 返回了工具调用 / 正文为空 ⇒ 抛 `CompactionError`（compaction.py:174-175），**不产出半份摘要**。
  - **落盘形状**（由 `ask._compact_if_needed()` 落盘，ask.py:212-220）：`{"summary": "<摘要正文>", "shadowed": [seq…], "shadowed_chars": <int>, "model": "<摘要模型名>", "usage": {…}?}`；`usage` 与 `loop/end.usage` **同形状**（来自新公开的 `loop.usage_payload()`，loop.py:133-143），省略表示端点未上报。
  - **回放口径（`history._compaction_plan()` / `_replay()`，history.py:169-194 / 197-259）**：被 `shadowed` 覆盖的 seq **全部跳过**；摘要在**该区间最旧那条 seq 的位置**出一条 `user` 消息（= 时序仍在原处，**不是**追加在末尾），正文由 `frame_summary()`（compaction.py:200-203）包上 preamble 与 `<compacted-summary>` 标签；**链式压缩**（后一次覆盖前一次区间、含前一条 `compaction` 记录自身）只出**最新**那份合并摘要；**无效记录**（`shadowed` 为空 / 摘要为空）忽略，不吞掉任何事件。`build_history()` = **先应用压缩、再兜底截断**（history.py:307-322）。
  - **调用时机与容错（`ask()`，ask.py:273-297）**：在**追加本轮 `user/message` 之前**做压缩（故被压缩的历史不含本轮问题）；只有「回放后的历史字符量 > 阈值」才尝试（`_history_chars()` / `_compact_if_needed()`，ask.py:158-227）；**压缩失败不打断提问**——`summarize_span()` 自身 fail-closed，`ask()` 只记 warning、按**未压缩**历史继续；压缩成功则**重新回放**，让本轮请求用上摘要视图。压缩与主回合**共用同一份 `system` + 工具集**（`build_loop()` 新增可选 `registry` / `system` 关键字参数，ask.py:119-120、133-136）以对齐 KV 前缀。
...
- **「清空对话」（生成中亦可用）**：本地面板清空 + **删掉当前库在 `agent.lastSessionByKb` 里的条目**（只删本库，不动别的库），且**顺带调 `agent_ask_cancel`**（避免丢弃结果却继续烧 token）。
...
- **前端呈现**：面板的 code 映射（`agent-panel.js:132-157`）**未新增、未改动**；LLM 失败一律经 `ask_failed` 兜底显示本地化文案 + **后端原文**（原文即含 URL 与"确定性失败，不会重试"，agent-panel.js:1373-1377）。
...
| `agent.err.<code>` | js/agent-panel.js:132-157（`ERR_KEYS`）、160（`GENERIC_CODES`）、680-708（`errorText`/`errorDetail`/`fullErrorText`）；zh-CN.js:653-669 | 对话面板把后端 agent RPC 的**稳定 code** 映射为文案（`no_kb`/`busy`/`no_base_url`/`unknown_session`/`session_failed`/**`cancel_failed`（M1 收尾新增）**/`MISSING_CREDENTIAL`/`RATE_LIMIT` …）；**未登记的 code 回退后端中文 `message`**；兜底 code（`ask_failed`/`config_error`）额外拼后端原文（LLM 失败原因只在原文里）。**`agent_usage_stats` 本轮新增的 `usage_failed` / `unknown_session` 走同一兜底**（前端暂未调用该 RPC，故未加映射） |
| `agent.history.*` | index.html:256-260（静态节点）；js/agent-panel.js:893-920（`renderHistory`）、949-968（`refreshHistory`）、976-1015（`loadSession`）；zh-CN.js:614-626 | M1c 历史会话下拉：`label`/`aria`/`none`（「（新会话）」）/`empty`（空列表禁用态占位）/`option`（`{preview}（{n} 轮）`）/`capped`（「（已截断）」，M1 收尾）；M1 收尾新增 `delete`/`deleteTitle`/`deleteConfirm`（「再点一次删除」）/`deleted`；选项文本用 `textContent` 写入（不注入 HTML） |
| `agent.stopped` / `agent.stopTitle` / `agent.status.stopped` | js/agent-panel.js:622-628（`messageEl` 的 `rec.stopped` 分支）、1402-1426（`stop()`）；index.html:266；zh-CN.js:610-613、636 | M1 收尾：生成中按钮文案「停止」（旧 `agent.abandon*` 已废除）、助手气泡后的「（已停止）」标注、状态行「已停止…」 |
| `agent.status.elapsed` | js/agent-panel.js:842-861（`tickWait`/`startWait`/`stopWait`）；zh-CN.js:632 | 等待计时后缀（`{n}s`），与 `agent.status.thinking` 由 `statusLine()` 拼成「生成中… Ns」 |
| `agent.statusBar.*` | index.html:278（静态节点，**空**）；js/agent-panel.js:754-830（`renderStatusUsage` 等）；zh-CN.js:642-652 | 本轮新增 9 键：`span` / `spanCache`（紧凑摘要，如 `↑8.7k ↓233 · 命中 62%`）、`line` / `cache` / `cacheUnknown` / `estimated` / `session` / `sessionCache` / `hint`（`title` 多行拆分，`\n` 连接）；命中率未知时**不渲染** `spanCache` 段 |
...
| 对话面板 9 个 RPC / 伪流式作业 / 真取消 / 配置与偏好读写 / 会话历史与删除 / 用量统计 | presentation/api/ui.py:1155-1400（配置 1155-1193、问答 1194-1225、取消 1226-1252、会话列表 1258-1300、载入 1301-1328、删除 1329-1367、用量统计 1368-1400）；services/agent/ask_stream.py:1-337；services/agent/llm/config.py:164-174、247-330；storage/ui_settings.py:58-75（顶层 `agent` 段） |
| **重试与失败分类**（确定性立即失败 / 瞬时重试 / 面板等待上限 / 错误串带重试次数） | services/agent/llm/errors.py:62、70、87、96、205、247、345-347；services/agent/llm/providers/openai_compatible.py:272-277、281、303、485-497；services/agent/llm/retry.py:52-56；services/agent/ask_stream.py:82-84；services/agent/loop.py:146-150、238-246、307-311（§2.16） |
...
| 对话会话 JSONL 存储 / 历史重建（压缩 + 回放与截断）/ 摘要扫描（去读放大）/ 续聊入口 | services/agent/session/store.py:56-66、169-237；services/agent/session/history.py:1-467（`COMPACTION` 99、`_compaction_plan` 169-194、`_replay` 197-259、`_truncate` 267-281、`conversation_events` 284-286、`replay_events` 289-304、`build_history` 307-322、`summarize_session` 343-345、`summarize_session_file` 359-407、`conversation_messages` 410-442）；services/agent/ask.py:113-316（`_history_chars` 158-165、`_compact_if_needed` 168-227、`replay` 246、`cancel` 247、回放注入 273-280、`messages=history` 315） |
| **长会话压缩（M2，`compaction` 事件）**（阈值/保留字符预算 / 区域选择与工具配对 / 摘要调用与 KV 前缀 / fail-closed / 落盘形状与回放口径） | services/agent/compaction.py:1-366（常量 103-171、`compact_threshold_chars` 187-189、`retain_chars` 192-197、`frame_summary` 200-203、`event_chars` 218-237、`balanced_cuts` 240-256、`select_span` 259-297、`summarize_span` 300-366）；services/agent/session/history.py:99、169-194、307-322；services/agent/ask.py:158-227、283-297；services/agent/loop.py:133-143（`usage_payload()`）；services/agent/session/store.py:49-50（`SESSION_FORMAT_VERSION`，**不 bump**）；tests/test_agent_compaction.py:1-465（§2.15、§7） |
| **会话检索（M2，`search_sessions` 工具）**（语义文本抽取 / 字面量匹配与防正则注入 / 摘要窗 / 过滤子 AND-OR / 四重有界化 / 字节级预筛 / 命中写成「会话 `<id>` 第 N 条」且不产生文档锚点） | services/agent/session/query.py:1-378（常量 93-105、`_STRUCTURAL` 108、`event_text` 125-146、`compile_text_pattern` 149-159、`snippet` 162-176、`SessionEventHit` 179-187、`SessionGroupHit` 190-199、`_Filters` 202-227、`_limit` 230-236、`_prefilter_ok` 239-246、`_scan` 249-299、`search_session` 306-331、`search_sessions` 334-377）；services/agent/tools/kb.py:73-80（`KB_TOOL_NAMES`）、83-84（`DEFAULT_SESSION_HITS` / `MAX_SESSION_HITS`）、446-482（`_search_session_history`）、566-592（工具声明）；tests/test_agent_session_query.py:1-355（§2.15、§6.9） |
| **真取消**（取消令牌 / 三个检查点 / 作业面收敛与幂等） | services/agent/loop.py:73-101（`CancelToken`）、216-218（`_cancelled`）、248-264（检查点② 与 `stream.close()`）、295-299（①）、380-386（③）；services/agent/ask_stream.py:100-215（`AskJob.cancel()` 178-191、`succeed/fail` 的取消守卫 144-174）、310-326（`AskJobManager.cancel()`） |
| 壳选择 / CLI（无 headless） | app/shell/__init__.py:12-24；cli/main.py:111-125 |
| 后端消息本地化（仅 check.issue） | js/app.js:258-268 |

## 7. 未证实 / 待确认

- ⚠️ **本轮（2026-09-19 后半：M2 会话检索 `search_sessions`）已取证 / 未取证**。已取证：
  ...
  - **修掉一个真 bug**：`limit=0` 原仍返回 1 条（上限判断从 `hits.append` 之后移到之前，query.py:286-287），由新写的 `test_search_session_respects_limit` 逮到。
...
- ⚠️ **上一轮（2026-09-19 前半：M2 长会话压缩 `compaction`）已取证 / 未取证**。已取证：
