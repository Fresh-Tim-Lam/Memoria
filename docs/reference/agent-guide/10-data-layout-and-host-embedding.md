...
### 2.15 应用内对话：10 个 RPC、会话事实源、多轮续聊、长会话压缩、工具结果裁剪、会话标题、会话检索与「停止/删除/改名」（2026-09-17 落地，2026-09-18 增补 M1c、M1 收尾与用量统计，2026-09-19 增补 M2 压缩、裁剪、会话标题与会话检索工具，2026-09-20 增补会话改名）

M1「对话」面板（**2026-09-17 第二轮起挂在右侧 `#-agent-dock`**，前端见 [01 篇 §2.4](./01-shell-and-layout.md)）共 10 个 RPC：
...
**2 个为 2026-09-18（M1 收尾）新增**（`agent_ask_cancel` ui.py:1226-1252、`agent_session_delete` ui.py:1329-1365），
**1 个为 2026-09-20（会话改名）新增**（`agent_session_rename` ui.py:1366-1419，历史行右键菜单的「重命名」；**追加**一条 `session/title`，最新者胜，见下行），
**1 个为 2026-09-18（用量可视化）新增的只读用量统计**（`agent_usage_stats` ui.py:1422-1454，见 §2.17）；
**行号重取说明（2026-09-20）**：`agent_session_rename` 插在 `agent_session_delete` 之后 ⇒ **`agent_usage_stats` 及其后的行号整体 +54**（`1368-1400` → `1422-1454`）；`agent_session_delete` 与更上方的方法**零漂移**（顺带按实读校正：`agent_session_delete` 旧记 `1329-1367`，实测 `def` 1329、末行 `}` 1365）。
...
**M2（2026-09-19）起**：会话事件面新增三条记录 —— 第 8 类 `compaction`（长会话压缩）、第 9 类 `compaction/prune`（工具结果裁剪）与第 10 类 `session/title`（**会话标题，log-only**），都是**纯追加**、**不新增 RPC、不 bump `SESSION_FORMAT_VERSION`**，分别见下方「长会话压缩」「工具结果裁剪」「会话标题」；同轮新增**第 6 个只读知识库工具** `search_sessions`（**也不新增 RPC** —— 它只在模型回合内部可用，前端不调用），见下方「会话检索工具」。**2026-09-20（上游读面移植）起**：知识库只读工具由 6 个增至 **9 个** —— 新增 `glob`（按模式列**工作区（今天 = 库根）**的文件，上限 100；`.memoria/**` 默认可见、`.git` 等 VCS 目录仍排除）、`grep`（Python `re` 逐行搜正文，按文件分组回 `Line N:`，上限 250 处/单行 2000 字节）与 `read_image`（**只做参数与格式校验；缺「多媒体眼睛」插件 ⇒ 明确返回 `UNSUPPORTED_IMAGE_INPUT`**）；`read_document` 同时获得 `offset`/`limit` 分页（默认且最多 2000 行、截断给续读提示）。**2026-09-22 起**：`read_document` 的**扩展名限制放开**（人报障「agent 读不了非 markdown 文件」）—— `.md`/`.markdown` 语义一字未改（frontmatter 剥离 + sidecar 知识点清单 + 行号锚点），**其它文本文件**（`.txt`/`.csv`/`.json`/源码…）也能读，走**纯文本窗口**（不剥 frontmatter、无知识点段，行号/续读口径同 `_read_window()`）；**二进制**（含 NUL 字节）明确拒 `BINARY_FILE` 并指路 `read_image`；编码 UTF-8（BOM 亦去掉）→ GB18030 兜底；单次只读前 1 MiB 且如实标注。见 [../../design/dsh-agent-port.md §6.16](../../design/dsh-agent-port.md)。**2026-09-20（会话查询五工具补全）起**：知识库只读工具由 9 个增至 **13 个** —— 新增 `session_event_search`（**一个会话内**按字面量检索事件，命中 ≤ **100** 条，可按 `seq_from`/`seq_to`/`time_from`/`time_to`/`event_types` 过滤）、`session_trace`（一个会话的**谱系**：祖先链由近及远 + 后代树）、`session_event_trace`（一条事件的**替换关系**：`compaction.shadowed` / `compaction/prune.pruned[].seq`）与 `session_event_read`（目标事件**完整未删节 JSON** + `before`/`after` 条相邻事件摘要，默认 0 / 上限 **50**），四者都**只读、作用域限本库、`session_id` 必填**（本地没有「当前会话」身份），参数/作用域非法 ⇒ `INVALID_ARGUMENTS`、目标不存在 ⇒ `NOT_FOUND`。见 [../../design/dsh-agent-port.md §6.17](../../design/dsh-agent-port.md)。**2026-09-20（引用板块 R 线）起**：知识库只读工具由 13 个增至 **15 个** —— 新增 `resolve_reference`（解析**一条**库内引用：`@相对路径` / `@[label](dsh-session:…)` / `[[…]]` / `文件:行号` / `![](...)` 五类，回「类型 / 归一化目标 / 状态 / 指向什么 / 歧义候选 / 建议下一步」）与 `audit_references`（按文档审计五类引用，逐条回「检查名 / 位置（`文件:行`）/ 目标 / 问题 / 严重级」，一次最多 100 条并在到顶时给「已达上限」提示）；两把都 `read_only=True`、全程零写入，多目标一律判 `ambiguous` 并回候选（**不猜**）、区间锚点回 `unsupported`（本地无读时投影）。见 [../../design/dsh-agent-port.md §6.18](../../design/dsh-agent-port.md)。
...
| `agent_session_rename` | `(session_id: str, title: str, kb_path: str \| None = None)` | **2026-09-20 新增（历史行右键菜单的「重命名」）**。**只追加**一条 `session/title`（`data = {title, message_seqs: [], source: {kind: "user"}}`）—— `title.fold_title()` 取**最后一条非空**（最新者胜）⇒ 列表/回放/模型输入三侧零改动（`title.py` 政策一节明言"将来接改名 UI 无需改回放"），**历史不被改写**（append-only）。写入范围与两道校验与删除同口径；标题先过 `title.clean_title_text()`（去控制序列/折叠空白），再按 `title.TITLE_MAX_BYTES`（120 字节）**拒绝**（不截断）。`{status:"ok", session_id, title}`；id 非法/会话不存在 ⇒ `code:"unknown_session"`；标题为空/过长/写入失败 ⇒ `code:"session_failed"` |
...
- **不存在跨库会话（显式声明）**：会话目录**按库分**（`<kb>/.memoria/agent/sessions/`），系统提示注入的也是**当前库**规范（`services/agent/prompt.py`：`build_system_prompt()` 按固定段落顺序「基础身份 → 运行环境 → 知识库指令文件 → 可用工具 → **用户引用（`@路径`）** → 回答要求」拼装，空段消失；其中「用户引用」段**只在本次请求的工具集含任一读取工具时**才追加 —— 语义移植自上游 `context/file-reference` 并按「有读取手段即注入」同构门控（2026-09-20 起名单为 `read_document` / `glob` / `grep` / `read_image`，旧口径只认 `read_document`），prompt.py:194-218（段落）、261（门控行）、385-394（门控名单与判定））。因此**一个会话只属于建立它的那个知识库**：切换知识库时必须作废当前会话态并由新库自己的 `lastSessionByKb` 条目恢复；**任何"把 A 库的会话 id 交给 B 库"的行为都是缺陷**（前端以 `sessionKb` 作兜底断言，绝不把别的库的 id 发给后端；后端也按传入 `kb_path` 解析会话文件）。将来若要做**全局助手**，应**另立一个不依赖知识库的会话位置**，而不是复用库内 `sessions/`。
- **会话事实源**：`<kb>/.memoria/agent/sessions/<session-id>.jsonl`（`services/agent/session/store.py:56-66`）。首行 header、其后每行一个事件（`seq` 连续、append-only、撕裂尾部对读者不可见）；由 `ask()` 写（ask.py:311-335）。事件类型既有 7 类（`user/message` / `assistant/message` / `tool/call` / `tool/result` / `step/start` / `step/error` / `loop/end`，写入点 loop.py:302-399 与 ask.py:335），**M2（2026-09-19）起新增第 8 类 `compaction`、第 9 类 `compaction/prune` 与第 10 类 `session/title`**（形状与回放口径见下「长会话压缩」「工具结果裁剪」「会话标题」）；**2026-09-22 起再增第 11、12 类 `command/run` 与 `command/done`** —— 斜杠命令（`services/agent/commands.py`，语义见 [../../design/dsh-agent-port.md §6.23](../../design/dsh-agent-port.md)）的**成对 log-only 生命周期**：`command/run {commandId, name, args?, source:{kind:"user"}}` → （命令自己的领域事件，如 `permission/preset`）→ `command/done {commandId, kind, text?}`；二者**不进模型上下文**（回放跳过、压缩预算计 0），**未注册的斜杠行一条都不落**（那行当普通提问走 `user/message`）。`session_id` 随 `agent_ask_poll` 的最终结果返回，面板状态行显示。
...
- **模型切换告知（2026-09-20，`core/agent` 的 `model-selection`；见 [../../design/dsh-agent-port.md §6.15](../../design/dsh-agent-port.md)）**：一轮的模型由配置在 `ask()` 入口读一次（`active_model`，ask.py:362），并在 `loop/end` 落盘（`loop/end.data.model`，loop.py:396）—— 故**会话可以跨模型**，而"哪一轮是哪个模型写的"从此可查。**告知**：续聊（`replay` 打开且会话文件已存在）时，若会话里**最后一条 `loop/end.model`** 与本轮模型不同，就在**本轮请求**里追加一条告知，插在用户文本之后、时间读数之前（`ask._model_notice()`，ask.py:479-495；文本由 `prompt.render_model_change_notice()`（prompt.py:371-382）产出，上游 `modelSwitchNotice()` 的中文落法）。逐字形态：`[模型已更换：本轮之前的助手回复由 <旧模型> 生成；本会话此后由 <新模型> 继续]`。**口径**：① 只进**本轮请求**、**不落盘**（会话 JSONL 里仍是用户输入的原文，与跨会话引用/时间读数同口径）；② 模型**没换**时不追加（不多出空行）；③ **老会话**（2026-09-20 之前落盘，无 `loop/end.model`）⇒ 按"未知"处理、不告知；④ `replay=False` 时请求里没有历史，同样不告知；⑤ 比对基准是"最近一轮"⇒ **换回旧模型会再次告知**。
...
- **会话检索工具（M2，2026-09-19 `search_sessions`；2026-09-20 补全为上游同款的 **5 个**：`session_event_search` / `session_trace` / `session_event_trace` / `session_event_read`）**：第 6 个只读知识库工具起（`search_sessions` 声明 `services/agent/tools/kb.py:581-607` —— **该锚点随 2026-09-20 读面移植重取，旧值 `:566-592`**；`KB_TOOL_NAMES` kb.py:73-80，2026-09-22 起为 **17** 个工具 —— 9 → 13 → 15 → 16（`propose_write`）→ **17**（`skill`，见 [§6.22](../../design/dsh-agent-port.md)）；R 线引用板块的 `resolve_reference` / `audit_references`（追加块 `kb.py:1608-2591`，见 [../../design/dsh-agent-port.md §6.18](../../design/dsh-agent-port.md)）），后端是新模块 `services/agent/session/query.py`（**纯标准库**，语义移植自上游 `packages/session-query/session-query` 的 `extraction.ts` + `filters.ts` + `tracing.ts` 与 `session-query/tool-session-query`，pin `0d1f5000`；**不吃** `session-query-sqlite`（本地不引索引）与 `session-log-export`（导出 UI）—— 见 [../../design/dsh-agent-port.md §6.9](../../design/dsh-agent-port.md) 与 **§6.17**）。用途是回答「我们之前聊过什么 / 上次说到哪」这类问题：它检索的是**过去与本知识库的对话记录**，与知识库文档检索（`search_kb`）**是两件事**。**§6.17 补齐的四个**（声明 `kb.py:1510-1605`、实现 `:1297-1363` / `:1366-1413` / `:1416-1464` / `:1467-1507`，四把 `Tool` 全 `read_only=True`）：`session_event_search`（单会话内事件级检索，命中 ≤ **100** 条，`seq`/`time`/`event_types` 过滤；文本里带一句「引用时写成『会话 `<id>` 第 N 条』」）；`session_trace`（会话谱系：祖先链由近及远 + 后代树；本地**无 `parentSession` 写入方** ⇒ 实际恒为「根 + 无后代」，工具文本如实说明）；`session_event_trace`（一条事件的替换关系；`sourceEventSeqs`/派生关系本地不落盘 ⇒ 文本明说「无从给出」，**不伪造「无」**）；`session_event_read`（目标事件**完整 JSON** ` ```json ` 块 + `之前的事件：`/`之后的事件：` 邻接摘要，窗口默认 0 / 上限 **50**）。
  ...
  - **偏差（引用时须一并给出）**：语料是**磁盘上的会话文件**（无 live 会话注册表、无 SQLite FTS 索引）；组间排序按 `modified_at` 倒序（**最近聊过的先出**）而非上游的**相关性**排序（本地无评分器）；`snippet` 取首个匹配点前后各 80 字符 + 省略号（**窗口算法未逐字对齐**上游）；不移植游标 `SessionSearchCursor`、宿主可观测性、调用方 `cwd` 授权面与 `searchTimeoutMs`（30s 协作截止）；工具面**不返回** `capped` 事实。**2026-09-20（§6.17）补的四个另有偏差**：`session_id` **本地必填**（上游可省略 = 当前会话，本地拿不到「调用方会话」身份）；`surfaces`/`availability`/`parent_session_ids`/`include_root_sessions` 四类过滤无本地对应；替换关系改由 `compaction`·`compaction/prune` 承载（本地无 `foldSurface` 表面折叠）；`sourceEventSeqs`/`derivedEventSeqs` **无从计算**（文本明说，不伪造「无」）；结果文本落为中文（`seq N` → 「第 N 条」、`Snippet:` → 「片段：」）；时间戳按毫秒截断（上游保留亚毫秒余数并夹紧端点）；错误码取本地码族（`INVALID_ARGUMENTS` / `NOT_FOUND`）；上游 `PROMPT_TEXT` 固定指引段仍未注入。
...
**3) `loop/end.usage` 载荷**（`loop.py::usage_payload()`，loop.py:133-143；`loop/end` 事件（loop.py:390-399，**载荷另带 `model`** = 本轮实际使用的模型，2026-09-20 起）、`AskResult.usage` 与**压缩事件 `compaction.usage`** **共用同一形状**）

`{prompt_tokens, completion_tokens, total_tokens, estimated, cache_read_tokens, cache_write_tokens, cache_miss_tokens}`
（前 4 键既有，后 3 键本轮起随 `loop/end` 与 `agent_ask_poll.usage` 一并返回；`AskResult` 的公开字段未改）。另有一个**兄弟键 `model`**（不在 `usage` 内）：2026-09-20 起 `loop/end.data.model` 记**本轮实际使用的模型**（`loop.py:396`），消费者是「模型切换告知」与将来的逐轮成本归属；旧读者忽略未知键即可（payload 只增不改）。

**4) RPC `agent_usage_stats`**（ui.py:1422-1454）—— **只读**，见 §2.15 表；
...
| `agent.history.*` | index.html:227-231（**2026-09-19 起 = dock 的「当前会话」行**）；js/agent-panel.js:1946-1949（`histTitle`）、1991-2059（`renderHistoryList`）、2151-2168（`refreshHistory`）、878-915（`loadSession`）；zh-CN.js:610-614 | 会话列表（**2026-09-19 起在左栏「历史」页签**）：`none`（「（新会话）」，dock 当前会话行的空态）/`capped`（「（已截断）」，M1 收尾）/`delete`（行内删除按钮）；行标题/元信息用 `textContent` 写入（不注入 HTML）。**2026-09-20 追加（历史行右键菜单）**：`agent.history.*` 新增 `rename` / `renameTitle` / `renamePh` / `renameFail` / `deleteTitle` / `deleteBody` / `busyLock`，`agent.historyList` 新增 `rightClickMore`（行悬浮提示「右键查看更多操作（重命名 / 删除）」—— 行内按钮退役后，它是菜单的**唯一发现入口**）；四个 `Object.assign` 块**一律追加在语言包文件末尾**（zh-CN.js / en.js 末端）⇒ 上半部所有 `<行号>` 锚点零漂移。**行内 `.-hist-del` 按钮退役**：`agent.historyList.deleteTitle|deleteArm` 两个键与其"两次点击确认（3s 复位）"不再被渲染（键留着、路径不可达，见 `agent-panel.js` 文件末尾追加块的说明） |
...
| `agent.status.elapsed` | js/agent-panel.js:840-859（`startWait`/`stopWait`；`tickWait` 自 2026-09-20 起由末尾块 2948-2954 包装）；zh-CN.js:626 | 等待计时后缀（`{n}s`）。**2026-09-20 起**不再与 `agent.status.thinking` 由 `statusLine()` 拼成一行写进 `#agent-status`：秒数由 `T("agent.generating")`（zh-CN.js:1320 / en.js:1408；**2026-09-20 重取**：原 `1322 / 1410` 指两语言包**末行**，本轮两包末尾各追加 8 行 `selAdd` 块 ⇒ 改指键声明行）单独渲染在**末条助手气泡尾**（`.-agent-generating`，app.css:5585-5599），状态行只留不带秒数的基词 |
...
| 对话面板 10 个 RPC（**+ 2026-09-22 追加在文件尾的四个审批档位 RPC：`agent_permission_get` / `_set_session` / `_set_default` / `agent_approval_answer`，见 §2.20**） / 伪流式作业 / 真取消 / 配置与偏好读写 / 会话历史与删除·改名 / 用量统计 | presentation/api/ui.py:1155-1454（配置 1155-1193、问答 1194-1225、取消 1226-1252、会话列表 1258-1300、载入 1301-1328、删除 1329-1365、**改名 1366-1419（2026-09-20 增）**、用量统计 1422-1454）；services/agent/ask_stream.py:1-400；services/agent/llm/config.py:164-174、247-330；storage/ui_settings.py:58-75（顶层 `agent` 段） |
...
| **Agent 用量**（`Usage` 缓存字段 / 端点字段映射 / `loop/end.usage` / 只读 RPC / 报告脚本） | services/agent/llm/types.py:109-152（`Usage`，`cache_miss_tokens`）、132-145（`plus()`）；services/agent/llm/usage.py:105-159（`UsageMeter`）；services/agent/llm/providers/openai_compatible.py:428-464（`_usage_from_wire`）；services/agent/loop.py:133-143（`usage_payload()`）、390-399（`loop/end`，含 `model`）；services/agent/usage_report.py:1-225；presentation/api/ui.py:1422-1454；scripts/benchmark/usage/report_usage.py（§2.17） |
| **系统提示组装**（固定段落顺序 / 「用户引用（`@路径`）」段与其门控 / 「时间上下文」段不按工具门控 / 模型切换告知文本） | services/agent/prompt.py:12（段落顺序）、194-218（`FILE_REFERENCE_SECTION`）、221-222（`_has_tool`）、225-283（`build_system_prompt`，文件引用门控 261-262、时间上下文段 263）、286-292（`prompt_debug_info`）、295-320（`TIME_CONTEXT_SECTION`）、323-355（`_local_now` / `format_time_context` / `render_time_context`）、358-382（`MODEL_CHANGE_NOTICE` / `render_model_change_notice`）；services/agent/ask.py:445（时间读数 + 模型告知追加在本轮请求末尾，不落盘）、479-495（`_model_notice`）；tests/test_agent_loop.py:489-506、539-603、606-709（§2.15、§6.15） |
| 对话会话 JSONL 存储 / 历史重建（压缩 + 裁剪 + 回放与截断）/ 摘要扫描（去读放大）/ 续聊入口 | services/agent/session/store.py:56-66、169-237；services/agent/session/history.py:1-541（`COMPACTION` 124、`_TITLE_TYPE_MARK` 141、`_tool_message` 168-183、`_compaction_plan` 203-229、`compaction_shadowed` 231-233、`_replay` 236-300、`_truncate` 308-322、`conversation_events` 325-327、`replay_events` 330-350、`build_history` 353-372、`summarize_session` 398-400、`_title_from_line` 409-412、`summarize_session_file` 423-481、`conversation_messages` 484-516）；services/agent/ask.py:110-444（`build_loop` 130-173、`_history_chars` 175-183、`_compact_if_needed` 185-274、`_append_fallback_title` 276-286、`_maybe_generate_title` 288-322、`ask` 324-444、`replay` 340、`cancel` 341、回放注入 374、`messages=history` 411）、462-495（`_last_recorded_model` / `_model_notice`：读上一条 `loop/end.model` 产模型切换告知） |
...
| **会话查询家族（M2，5 个只读工具：`search_sessions` + `session_event_search` / `session_trace` / `session_event_trace` / `session_event_read`）**（语义文本抽取 / 字面量匹配与防正则注入 / 摘要窗 / 过滤子 AND-OR / 四重有界化 / 字节级预筛 / 命中写成「会话 `<id>` 第 N 条」且不产生文档锚点 / 事件精读窗口 ≤ 50 / 等价「替换关系」取自 `compaction`·`compaction/prune`；`session_trace` 的谱系恒为「根 + 无后代」、引用关系字段 `None`） | services/agent/session/query.py:1-679（常量 95-107、`_STRUCTURAL` 110、`event_text` 127-148、`compile_text_pattern` 151-161、`snippet` 164-178、`SessionEventHit` 181-190、`SessionGroupHit` 192-202、`_Filters` 204-230、`_limit` 232-239、`_prefilter_ok` 241-249、`_scan` 251-302、`search_session` 308-334、`search_sessions` 336-380；**2026-09-20 新增块** 常量 401-409、`SessionQueryNotFound` 412、结果 dataclass 417-459、`require_session` 462-470、`_events_of` 473-476、`_index_of` 479-483、`_covered_seqs` 486-497、`_replacement_maps` 500-516、`_window` 519-525、`read_event` 528-552、`trace_event` 555-573、`_header_of` 576-584、`_parent_of` 587-592、`_lineage_record` 595-602、`session_lineage` 605-662、`__all__ +=` 665-679 —— 旧锚 `1-380` 随本轮重取）；services/agent/tools/kb.py:73-80（`KB_TOOL_NAMES`，**15** 个名字）、83-84（`DEFAULT_SESSION_HITS` / `MAX_SESSION_HITS`）、446-482（`_search_session_history`）、581-607（`search_sessions` 声明）、**1165-1605（会话查询家族追加块：`SESSION_EVENT_HITS_CAP` 1185、`_ISO_TIMESTAMP_PATTERN` 1188、`_SessionArgError` 1193、`_epoch_ms` 1208-1240、`_seq_arg` 1243、`_event_types_arg` 1252、`_session_arg` 1266、`_title_of` 1274、`_seq_text` 1282、`_neighbour_line` 1286、四个实现 1297-1363 / 1366-1413 / 1416-1464 / 1467-1507、声明工厂 `_session_query_tools` 1510-1605）**、**1608-2591（R 线引用板块追加块：常量 1640-1647、三条正则 1669-1686、`_reference_normalize` 1725、`_reference_converge_file` 1749、五个解析体 `_resolve_file_reference` 1837 / `_resolve_session_reference` 1917 / `_resolve_kp_link_reference` 1970 / `_resolve_anchor_reference` 2015 / `_resolve_image_reference` 2099、分派表 `_REFERENCE_RESOLVERS` 2187、`_detect_reference_kind` 2196、工具体 `_resolve_reference` 2214-2235、`_audit_document` 2272-2466、`_audit_references` 2468-2521、声明工厂 `_reference_tools` 2526-2591；`build_kb_tools` 末位拼接在 `:665`）**；tests/test_agent_session_query.py:1-355、tests/test_agent_session_query_trace.py:1-522、tests/test_agent_tools_read.py:416（工具清单断言，§2.15、§6.9、§6.17） |
...
- ⚠️ **本轮（2026-09-18：M1 收尾打磨四件事——真取消 / 会话列表去读放大 / 恢复上次会话 / 删除会话）已取证 / 未取证**。已取证：
  ...
  - **会话删除（`agent_session_delete`）**：单测覆盖正常删除（删除后知识库逐文件 SHA256 只少该 JSONL）与 8 种非法/未知 id（`../decoy`、`..\decoy`、`a/b`、`..`、空串、空白、中文 id、不存在 id）——**全部结构化返回 `code:"unknown_session"` 且知识库快照逐字节不变**。**2026-09-20 起**入口从行内按钮改为**历史行右键菜单 + 应用内确认弹窗**（`tests/test_agent_panel_token.py::test_history_row_context_menu_wiring_is_intact` 静态守住接线）；端到端在真实浏览器里验证「右键 → 菜单出现 → 点删除 → 弹确认框 → 取消后行数不变」（详见 [01 篇 §7](./01-shell-and-layout.md)）。
  - **会话改名（`agent_session_rename`，2026-09-20）**：单测 `tests/test_agent_history.py::test_agent_session_rename_appends_title_only` 断言「只追加一条 `session/title`、既有事件逐条逐字节不变、只动目标文件、`fold_title()` 折叠出最新标题（连改两次 = 最新者胜）」；`test_agent_session_rename_rejects_bad_input` 断言空/超长标题与 6 种非法 id **全部拒写**（知识库快照逐字节不变）。端到端在真实浏览器里验证「右键 → 重命名 → 弹窗预填当前标题 → 确认后行标题与 `agent_sessions_list.title` 同步更新、`turn_count` 不变」。
...
## 2.20 审批档位（会话级旋钮）+ 逐条确认（2026-09-22，语义移植自上游 `interaction/permission-presets`）

**两件事、两个事实源**：

- **档位 = 会话事件（可回放）**：切换档位会往本会话 JSONL 追加两类**整值事件**（各取最后一条，生效值 = 最后一个覆盖事件 ?? 组合默认）——
  - `permission/preset`：`{"preset": "manual-approval" | "auto-approval" | "all-access"}`（**用户意图**，log-only）；
  - `approval/policy`：`{"policy": "ask" | "auto" | "never"}`（真正控制执行的**旋钮**）。
  两者**不进模型请求**：`build_history()` 只读 `content`/`tool_calls`；会话检索 `query.event_text()` 与压缩计量 `compaction.event_chars()` 对未知类型一律返回 空串/0（故新事件不会污染检索与 token 预算）。
  **会话开局钉盘**：每次 `ask()` 都会调 `permission_presets.pin_and_current()` —— 缺 `permission/preset` 就补一条当前生效档、缺 `approval/policy` 就补其 bundle 值（对齐上游 `pinInitialPermission()`：**只补缺失的事实**，已有覆盖不动）。
- **默认档 = 按 agent 的配置**：`config/agent.json` 的 `permission` 段（形状 `{agent_id: 档位名}`，当前只有主 agent `main`）。**新会话**用它的值；**老会话**仍以自己日志里的覆盖事件为准。值非法/读失败一律回落 `DEFAULT_PRESET = "auto-approval"`（配置文件是**用户可手改的边界**，不抛）。
- **派生态 `custom`**：当前旋钮不匹配任何档时由 `derive()` 算出 ⇒ **可展示、绝不可作切换目标**，也**不进**事件载荷（`set_preset()` 对它直接报错）。

**三个 RPC（+1 个回填）**（`presentation/api/ui.py` 文件尾追加、运行期挂到 `UIAPI`）：

| RPC | 作用 | 失败面 |
|---|---|---|
| `agent_permission_get(kb_path?, session_id?)` | 读档位面 `{status, current, agent, default, options:[{value,name,description}]}`；无会话时 `current` = 该 agent 的默认档 | `permission_failed`（读取异常） |
| `agent_permission_set_session(preset, kb_path?, session_id?)` | 切**当前会话**的档（落上述两条事件；已是生效档 ⇒ **不落任何事件**） | `no_kb` / `no_session`（含"会话文件不存在"）/ `bad_preset`（未知名与 `custom`）/ `permission_failed` |
| `agent_permission_set_default(preset, agent?)` | 改某 agent 的**默认档**（写 `config/agent.json: permission.<agent>`，只影响以后新建的会话） | `bad_preset` / `permission_failed` |
| `agent_approval_answer(call_id, outcome, kb_path?)` | 回填一次**逐条确认**的裁决：`allowed-once` / `rejected` / `cancelled` | `no_kb` / `bad_outcome`（未知名，及 `unavailable` —— 那是内部终态、不是人能点的按钮）；未知/已处理的 `call_id` ⇒ `{status:"ok", answered:false}`（幂等） |

**轮询载荷新增键**：`agent_ask_poll` 的返回**追加** `pending_approvals`（只增不改，形状 `[{id, tool, arguments, reason, created_at}]`，`id` = 模型给的 tool call id、`created_at` = 后端登记待批项的毫秒时间戳）。前端按 **`id@created_at` 复合键**建卡 —— call id 只保证**一轮内**唯一，跨轮复用同一个 id 时若按 id 单键去重，新的待批项会被**静默跳过**（挂起 120s 后判拒绝 = **写入静默死掉**；L4 实测事故）。

**执行侧**（不在数据面，但决定"谁来问"）：

- 工具分发**之前**问策略（`tools/registry.py` 的 `approval` 闸，位置未变）；策略实例由 `permission_presets.policy_for(档位, kb_path)` 决定：`ask` ⇒ `AskPolicy`（逐条确认）、`auto` ⇒ `GuardedPolicy`（常规写放行、**风险条件**命中才问）、`allow-all` ⇒ `DefaultApprovalPolicy`（不问全放行；2026-09-22 前这个值叫 `never`，因与上游同名反义而改名，旧会话事件仍只读兼容）。
- **读后写守卫**（2026-09-22 补，`services/agent/observation.py`，语义移植上游 `fs/fs-observation-policy`）：`read_document` / `read_kp` 成功即记 `present@sha256`（读到不存在记 `confirmed absent`），**只有 `create_file` 免读**；其余 op 未读 ⇒ `FS_NOT_OBSERVED`、读到过但当时不存在 ⇒ `FS_NOT_FOUND`、读后内容变了 ⇒ `FS_STALE_VERSION`。观察表**进程内、不跨会话**（resume 后须重读），写成功后由 `_refresh_observations()` 刷新版本（上游 `fs/observed` 同口径，避免模型改完撞自己造成的 STALE）。
- **风险条件**（`approvals.RISKY_OPS` + `write_is_risky()`）：写计划含 `rename_file` / `delete_file` / `move_file`（**动"路径/归属"坐标系、或整篇消失**），或**形状认不出**（缺 `ops` / 空表 / 项没 `op` 动词 / 换了新写工具）⇒ 一律按风险；批量大小**不算**风险。**改正文的三个 op（`replace_lines` / `insert_lines` / `delete_lines`）都不算风险** —— 判据是"不可逆"，而它们有逐字 `expect` + 写前备份 + 整批撤销，可逆性与"整段替换成空串"没有区别（2026-09-22 订正：`delete_lines` 原先在表内，导致"整理残渣"这类纯删行任务每批都弹确认卡、没人点就挂满 120s 后按拒绝关闭，真机实测连续 3 次超时）。
- **应答者 = 进程内信箱**（`services/agent/approval_bridge.py`，不落盘）：`AskJobManager._run()` 里 `attach(库根)` ⇒ 工具体线程挂起等待 ⇒ 面板轮询看见待批项、点按钮 ⇒ `agent_approval_answer` 唤醒。四条关闭路径全部 **fail-closed**：超时（`APPROVAL_TIMEOUT_S = 120`）⇒ `unavailable`；本轮被取消 ⇒ `cancelled`；**未挂载**（CLI `scripts/agent_ask.py` 不走作业面）⇒ 无应答者 ⇒ **立即拒绝、不空等**；应答者返回词汇外取值或抛错 ⇒ `unavailable`。
