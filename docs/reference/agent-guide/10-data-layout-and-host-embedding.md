...
### 2.15 应用内对话：9 个 RPC、会话事实源、多轮续聊、长会话压缩与「停止/删除」（2026-09-17 落地，2026-09-18 增补 M1c、M1 收尾与用量统计，2026-09-19 增补 M2 压缩）
...
**M2（2026-09-19）起**：会话事件面新增 `compaction` 记录（**纯追加**，**不新增 RPC、不 bump `SESSION_FORMAT_VERSION`**），见下方「长会话压缩」。
...
- ⚠️ **本轮（2026-09-19：M2 长会话压缩 `compaction`）已取证 / 未取证**。已取证：
