...
| **系统提示组装**（固定段落顺序 / 「用户引用（`@路径`）」段与其门控 / 「时间上下文」段不按工具门控） | services/agent/prompt.py:12（段落顺序）、194-218（`FILE_REFERENCE_SECTION`）、221-222（`_has_tool`）、225-283（`build_system_prompt`，文件引用门控 261-262、时间上下文段 263）、286-292（`prompt_debug_info`）、295-320（`TIME_CONTEXT_SECTION`）、323-355（`_local_now` / `format_time_context` / `render_time_context`）；services/agent/ask.py:421（读数追加在本轮请求末尾，不落盘）；tests/test_agent_loop.py:489-506、539-603（§2.15） |
