...
| UI 偏好 | `config/ui-settings.json`（`layout` 段 + **M1 收尾新增的顶层 `agent` 段** `{lastSessionId}`） | 前端 → `save_ui_settings` | 可删（回默认值） | ❌ |
...
- **已知顶层键**：`layout`（`sidebarWidth` / `agentDockWidth` / `agentDockCollapsed` 等）、`last_kb_path` / `recent_kbs`、主题与显示设置、`check`/`graph`/`search` 设置段，以及 **M1 收尾新增的 `agent` 段**（`{lastSessionId}` —— 对话面板的「上次会话」，见 §2.15）。**浅合并的坑**：直接传 `{layout:{agentDockWidth}}` 会整段替换 `layout`（抹掉 `sidebarWidth`）；`{agent:{lastSessionId}}` 同理会替换 `agent` 段。前端因此统一走「先 `get_ui_settings` 读旧段 → 合并 → `save_ui_settings`」（agent-panel.js:293-322 的 `saveDockLayout()` 与 `saveAgentPrefs()`）。
...
- **偏好落点（M1 收尾）**：`config/ui-settings.json` 的**顶层 `agent` 段** `{lastSessionId}`——对话面板的「上次会话」；提问成功 / 手动载入 / 自动恢复时写入，**清空对话时置空**。前端 `saveAgentPrefs()`（agent-panel.js:311-322）先读旧 `agent` 段再合并写回，**绝不覆盖 `layout` 段**（后端浅合并口径见 §2.9）。
...
- **「清空对话」（生成中亦可用）**：本地面板清空 + 偏好 `agent.lastSessionId` 置空，且**顺带调 `agent_ask_cancel`**（避免丢弃结果却继续烧 token）。
- **恢复上次会话（M1 收尾）**：面板打开（`init` / dock 展开）或**切换知识库**（`app.js` 在 `initKb:452`/`openKbAt:560`/`closeKb:598` 调 `panel.onKbChanged()`）时，若 `agent.lastSessionId` 指向的会话文件仍存在 ⇒ 自动 `agent_session_load` 并设为当前会话（下一句即续聊）；**不存在则静默忽略**（不报错、不提示）。
