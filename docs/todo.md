...
T01–T03 整段收口（右键重命名/删除/新建、重命名全库引用级联），已按规则 8.4 出表，见 §10。
...
| AG06 | **UI 视觉语言对照与改进**（Memoria 借 dsh 的观感）：A 档（5 项零风险）已落地；B 档（圆角收敛/0.5px 描边/字号上移/两档阴影/细滚动条）、C 档（别名令牌层/字号可调轴/`app.css` 拆分）待拍板 | 🔄 K1 **A 档已完成**（设计稿 [design/ui-visual-language.md](design/ui-visual-language.md) §4-A 实施记录：新增 `--font-sans`/`--font-mono` 唯一来源 + 9 处改 `var()`、`prefers-reduced-motion` 兜底、120ms 四属性过渡、`:focus-visible` 焦点环、树选中并入 `--accent-soft`；**④ 焦点环真机观感未取证**）；**U2–U6 待选** |
| AG07 | **锚点匹配的遗留**（助手气泡 `文件:行号`）：① 含空格未加引号的路径被腰斩；② 非 `.md` 不匹配（按设计）；③ 路径中段含全角括号退化为 basename；④ **区间只跳起始行**（需 `app.js` 接"到行"，会推动其行号锚点）；⑤ `.md:L7` 不匹配 | ⏳ K2（①②③⑤ 的正解 = 用**当前库文件清单**收敛候选路径；现状与实测见 agent-guide/01 §7；本轮已修：中文标点吞路径 + 全角冒号 + 区间显示） |
