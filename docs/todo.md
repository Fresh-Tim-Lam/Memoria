...
| AG03   | dsh M2 `session-reference`                                                                                                                                             | ✅ 代码完成（§6.12；见 §4.2）                                                                                                                   |
...
| <br /> | agent输出内容的时候，memoria做一层中间缓冲，这个既起到缓冲作用，又可以让输出的内容逐行渲染（对于一些特殊的语法块，需要特殊处理保证能渲染才输出，比如表格，给完一行可以直接渲染一行表格，但是对于\`\`\`得等到\`\`\`才能判断，这期间可以给一个转圈进度条，甚至以后给图片渲染支持（纳入todo))，而不是输出完才渲染 | <br />                                                                                                                                 |
| <br /> | 没有看到agent的思考过程，是本来就没有吗                                                                                                                                                 | <br />                                                                                                                                 |
