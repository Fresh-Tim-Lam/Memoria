...
| P07 | UI 视觉收敛（2026-09-19 用户直接指令）：滚动条改细+按需显形、标签栏滚轮横向滚动、圆角 4/6/8/12/999、中性描边 0.5px、阴影三档+模态背板 | 🔄 K2（`app.css:5078-5168` 两个末尾追加块 + 全库原位数值替换；harness 8651/8652 实测：轨道 10px/可见滑块 2px、8 标签溢出滚轮 `scrollLeft 0→240`、圆角 1/2/3/5/7/10px 清零、三档阴影令牌解析。**未取证**：**B-7 的 0.5px 在 DPR=1 下被上取整为 1px ⇒ 视觉 no-op**（仅 DPR≥2 有收益，替代方案待定）、真实 hover/滚轮、真机观感。详见 [design/ui-visual-language.md §4 实施记录 A/B](design/ui-visual-language.md)） |
...
| # | 任务 | 现状 |
| --- | --- | --- |
| AG01 | 引用 agent 回复内容再追问（选中/引用某条回复当下一轮显式上下文） | 💡 K1 需求待规格化（粒度/入口/请求形状未定） |
